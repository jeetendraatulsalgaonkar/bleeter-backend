...
import com.abc.bleeter.bleeterbackend.model.BleetObject
...
class BleeterBackendControllerTest (@Autowired val mockMvc: MockMvc) {
    ...
    @Test
    fun getBleets_NoBleetsReturned() {
    ...
    fun getBleetsByBleeter(user: String) {
        ...
        val bleet1 = Bleet()
        bleet1.id = UUID.randomUUID().toString()
        bleet1.bleetMessage = "Twitter without T"
        bleet1.bleetUser = "ElonMusk"
        bleet1.bleetTimestamp = Timestamp.valueOf("2022-04-21 19:49:51")
        bleets.add(bleet1)
    ...
    companion object {
        ...
        fun getBleets() : Stream<Arguments> {
            val bleets = Bleets()
            var bleetList: MutableList<BleetObject> = ArrayList()
            var bleet1 = BleetObject()
            bleet1.id = UUID.randomUUID().toString()
            bleet1.bleetMessage = "Twitter without T"
            bleet1.bleetUser = "ElonMusk"
            bleet1.bleetTimestamp = "2022-04-21 19:49:51"
            bleetList.add(bleet1)
            bleets.addAllBleetObjects(bleetList)
            return Stream.of(
                Arguments.of(bleets)
            )
        }
